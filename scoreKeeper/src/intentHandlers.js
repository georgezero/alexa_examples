...
var registerIntentHandlers = function (intentHandlers, skillContext) {
    ...
    intentHandlers.AddScoreIntent = function (intent, session, response) {
        ...
        var playerName = textHelper.getPlayerName(intent.slots.PlayerName.value),
            ...
            scoreValue;
        if (!playerName) {
            response.ask('sorry, I did not hear the player name, please say again?');
            return;
        }
        scoreValue = parseInt(score.value);
        if (isNaN(scoreValue)) {
            console.log('Invalid score value = ' + score.value);
            response.ask('sorry, I did not hear the points, please say again?');
            return;
        }
        storage.loadGame(session, function (currentGame) {
            var targetPlayer, speechOutput = '', newScore;
            if (currentGame.data.players.length < 1) {
                response.ask('sorry, no player has joined the game yet, what can I do for you?');
                return;
            }
            ...
            if (!targetPlayer) {
                response.ask('Sorry, ' + playerName + ' has not joined the game. What else?');
                return;
            }
